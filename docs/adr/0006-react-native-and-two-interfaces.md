...
**Capacitor.** The cheapest route that *can* schedule one: the Screens, the token system, the canvas Share Card and the Playwright suite all keep working inside a WebView, and only `handoff.ts` and `storage.ts` grow a native branch. It was the lower-cost option and it was not taken. What it does not give is platform navigation — a native header and the iOS interactive pop, both free in the chosen stack, and Android predictive back, which a WebView can never have because it owns no native back stack. This app needs that more than most, because ADR 0004 spent an entire decision on the fact that a standalone web app draws its own way out or has none. It also carries a Play Store review risk that a bare WebView wrapper does not clear by itself.
...
Android predictive back is a capability this decision *keeps possible*, not one it delivers, and the paragraph above should not be read as banking it. `android.predictiveBackGestureEnabled` defaults to false; `react-native-screens` does not implement predictive back in its current stable major, and its maintainer has said it is unlikely ever to land there; and enabling the flag with Expo Router's default stack is an open bug where the gesture leaves the app instead of popping a Screen. Revisit when the library ships support. It is not initial scope, and ADR 0004's navigation design does not depend on it — the Android back gesture already works, it simply does not animate predictively. Measured 2026-08-20 against Expo SDK 57; see `docs/research/expo-notifications-and-predictive-back.md`.
